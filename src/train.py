...
fixed_noise = torch.randn(NOISE_WIDTH, 100).cuda()
...
dataset = dset.ImageFolder(Path.cwd() / "assets" / "img", transform=transform)
...
for epoch in range(n_epoch):
    for i, (data, _) in enumerate(dataloader):
        ...
        netD.zero_grad()
        data = data.cuda()
        batch_size = data.size(0) * CHANNEL_NUM
    ...
    training_progress_images_list = save_gif(
        training_progress_images_list, fake
    )  # Save fake image while training!

    # Check pointing for every epoch
    torch.save(
        netG.state_dict(),
        Path.cwd() / "assets" / "checkpoint" / f"netG_epoch_{epoch}.pth",
    )
    torch.save(
        netD.state_dict(),
        Path.cwd() / "assets" / "checkpoint" / f"netD_epoch_{epoch}.pth",
    )
